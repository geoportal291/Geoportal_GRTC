...
const getAllSenales = async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'SELECT * FROM senales_reguladoras WHERE id_proyecto = $1 ORDER BY id_senal_reguladora DESC',
            [projectId]
        );
        res.json(result.rows);
    } catch (error) {
        console.error('Error al obtener señales reguladoras:', error);
...
const createSenal = async (req, res) => {
    ...
    try {
        const result = await pool.query(
            `INSERT INTO senales_reguladoras 
            (codigo, progresiva, lado, tipo, clasificacion, material, 
        ...
        console.error('Error al crear señal reguladora:', error);
        res.status(500).json({ message: 'Error interno del servidor' });
    }
};

const processExcel = async (fileBuffer, projectId, entregableNum, utmZone) => {
    console.log(`DEBUG: Iniciando processExcel (REGULADORAS) para proyecto ${projectId}, Zona: ${utmZone || 'Defecto 18L'}`);

    // Parse UTM Zone if provided
    let zoneNum = 18;
    let zoneLetter = 'L';
    if (utmZone) {
    ...
    if (headerRowIndex === -1) {
        console.warn('DEBUG: "Ubicación (km)" header not found using fallback.');
        headerRowIndex = 1;
        colMap.codigo = 0;
        colMap.tipo = 1;
        colMap.progresiva = 3;
    ...
    try {
        ...
        await client.query('DELETE FROM senales_reguladoras WHERE id_proyecto = $1', [projectId]);
        ...
        for (let i = startRowIndex; i < rawData.length; i++) {
            ...
            // Filter: Only process "Señal Reguladora"
            if (clasificacion && typeof clasificacion === 'string' && clasificacion.toLowerCase().includes('reguladora')) {
                // Keep it
            } else {
                // Check if it might be reguladora by other means? For now loose check or strict?
                // Original logic for Informativa was strict. I will stick to strict for now but maybe 'Reguladora' check.
                if (!(clasificacion && clasificacion.toLowerCase().includes('reguladora'))) {
                    // But wait, what if the excel has "Señales Reguladoras" as a title logic?
                    // The classification is usually distinct.
                    // Let's assume classification column contains 'Reguladora'.
                    continue;
                }
            }
            ...
            if (typeof coord2 === 'string') coord2 = parseFloat(coord2.replace(/,/g, '')); // Fixed typo from Informativas? No, Informativas had coord2 = parseFloat(coord2.replace...)
        ...
        console.log(`DEBUG: Inserted ${insertedCount} Senales Reguladoras.`);
...
const deleteSenal = async (req, res) => {
    const { id } = req.params;
    try {
        await pool.query('DELETE FROM senales_reguladoras WHERE id_senal_reguladora = $1', [id]);
...
const updateSenal = async (req, res) => {
    const { id } = req.params;
    try {
        // Placeholder, implement like create but with UPDATE
        const {
            codigo, progresiva, lado, tipo, clasificacion, material,
            latitud, longitud, altitud, observaciones,
        ...
        await pool.query(
            ...
            WHERE id_senal_reguladora = $12`,
            [codigo, progresiva, lado, tipo, clasificacion, material,
                latitud, longitud, altitud, observaciones, panel_fotografico_codigo, id]
        );

        res.json({ message: 'Señal actualizada correctamente' });
    } catch (e) {
        console.error(e);
