...
const getAllSenales = async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'SELECT * FROM senales_informativas WHERE id_proyecto = $1 ORDER BY id_senal_informativa DESC',
            [projectId]
        );
        res.json(result.rows);
    } catch (error) {
        console.error('Error al obtener señales informativas:', error);
...
const createSenal = async (req, res) => {
    ...
    try {
        ...
        console.error('Error al crear señal informativa:', error);
        res.status(500).json({ message: 'Error interno del servidor' });
    }
};

const processExcel = async (fileBuffer, projectId, entregableNum, utmZone) => {
    console.log(`DEBUG: Iniciando processExcel para proyecto ${projectId}, Zona: ${utmZone || 'Defecto 18L'}`);
    ...
    try {
        ...
        for (let i = startRowIndex; i < rawData.length; i++) {
            ...
            // Filter: Only process "Señal Informativa"
            if (!clasificacion || typeof clasificacion !== 'string' || !clasificacion.toLowerCase().includes('informativa')) {
...
const deleteSenal = async (req, res) => {
    const { id } = req.params;
    try {
        await pool.query('DELETE FROM senales_informativas WHERE id_senal_informativa = $1', [id]);
