## Corrección y Simplificación de la Configuración de Cálculos para Granulometría
...
2.  **Error de `index out of range` y `Value expected (char 9)`**: La fórmula de `porcRetAcumulado` ha sido simplificada para eliminar el operador ternario problemático, confiando en el `defaultValue` de la función `get` para manejar el primer elemento de la tabla.
...
4.  **Corrección de escape de comillas**: Se ha revisado la estructura del JSON para asegurar que las comillas simples dentro de las fórmulas (`'n200'`, `'n4'`) sean interpretadas correctamente por `math.evaluate` y no como `''n200''`.
...
```
**Nota sobre el SQL:** Las comillas simples dentro del JSON deben escaparse duplicándolas (`''`) para que PostgreSQL lo interprete correctamente como una cadena literal. He revisado cuidadosamente que las comillas dentro de las fórmulas (`'n200'`, `'n4'`) estén correctamente escapadas para el SQL, pero que el JSON resultante sea válido para `math.evaluate`.
