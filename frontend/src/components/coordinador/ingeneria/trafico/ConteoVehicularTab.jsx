  ...
  return (
    <div className="estacion-control-tab-wrapper">
                ...
                <div className="stations-grid" style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'flex-start', overflowX: 'auto', paddingBottom: '10px', width: '100%', gap: '10px' }}>
                  {Object.keys(stationData).map((stationId) => (
                    ...
                    >
                      ...
                      {!(stationData[stationId].info && stationData[stationId].info.imagenes && stationData[stationId].info.imagenes.filter(item => item.source_type === 'conteo_vehicular_image' || item.source_type === 'conteo_vehicular_file').length > 0) && (
              ...
              <div ref={photosRef}>
                <div style={{display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px'}}>
                    ...
                    {currentStationData && currentStationData.info && currentStationData.info.imagenes &&
                      Object.values(currentStationData.info.imagenes.filter(image => image.source_type === 'conteo_vehicular_image').reduce((acc, image) => {
              ...
              <div ref={dataCollectionRef}>
                {currentStationData && currentStationData.info && currentStationData.info.imagenes && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    {currentStationData.info.imagenes.filter(file => file.source_type === 'conteo_vehicular_file').length > 0 ? (
                      currentStationData.info.imagenes.filter(file => file.source_type === 'conteo_vehicular_file').map((file, index) => (
      ...
      <ImageFileExplorerModal
        ...
        data={stationData}
        allowedSourceTypes={['conteo_vehicular_image', 'conteo_vehicular_file']}
...
export default ConteoVehicularTab;
