  ...
  return (
    <div className="estacion-control-tab-wrapper">
                ...
                <div className="stations-grid" style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'flex-start', overflowX: 'auto', paddingBottom: '10px', width: '100%', gap: '10px' }}>
                  {Object.keys(stationData).map((stationId) => (
                    ...
                    >
                      ...
                      {!(stationData[stationId].info && stationData[stationId].info.imagenes && stationData[stationId].info.imagenes.filter(item => item.source_type === 'encuesta_origen_destino_image' || item.source_type === 'encuesta_origen_destino_file').length > 0) && (
              ...
              <div ref={dataCollectionRef}>
                {currentStationData && currentStationData.info && currentStationData.info.imagenes && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    {currentStationData.info.imagenes.filter(file => file.source_type === 'encuesta_origen_destino_file').length > 0 ? (
                      currentStationData.info.imagenes.filter(file => file.source_type === 'encuesta_origen_destino_file').map((file, index) => (
      ...
      <ImageFileExplorerModal
        ...
        data={stationData}
        allowedSourceTypes={['encuesta_origen_destino_image', 'encuesta_origen_destino_file']}
...
export default EncuestaOrigenDestinoTab;
