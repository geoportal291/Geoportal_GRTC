...
const ReportCensoDeCargasTab = ({ stationData, selectedStation, handleStationSelect, showRoute, setShowRoute, showTraffic, setShowTraffic, refreshStationData, setIsLoadingImages, isLoadingImages, isNavbarExpanded }) => {
  ...
  const [origenDestinoLabels, setOrigenDestinoLabels] = useState([]);
    ...
    const [censoCargasBarData, setCensoCargasBarData] = useState(null); // Nuevo estado para el gráfico de barras de censo de cargas
    const [censoCargasPieData, setCensoCargasPieData] = useState(null); // Nuevo estado para el gráfico de torta de censo de cargas
    const [censoCargasCE3BarData, setCensoCargasCE3BarData] = useState(null); // Nuevo estado para el gráfico de barras de censo de cargas CE3
    const [censoCargasCE3PieData, setCensoCargasCE3PieData] = useState(null); // Nuevo estado para el gráfico de torta de censo de cargas CE3
    ...
    const stationChartConfig = {
      'E-01': {
          ...
          censoCargas: { // Configuración para censo de cargas CE2
            sheetName: 'R. CARG. E1. CE2',
            labelsRange: 'I6:I19',
            dataRange: 'J6:J19',
            pieDataRange: 'K6:K19',
          },
          censoCargasCE3: { // Nueva configuración para censo de cargas CE3
            sheetName: 'R. CARG. E1. CE3',
            labelsRange: 'I6:I19',
            dataRange: 'J6:J19',
            pieDataRange: 'K6:K19',
          }
      ...
      'E-04': {
          censoCargas: { // Configuración para censo de cargas CE2 para E-04
            sheetName: 'R. CARG. E4. CE2',
            labelsRange: 'I6:I19',
            dataRange: 'J6:J19',
            pieDataRange: 'K6:K19',
          },
          censoCargasCE3: { // Configuración para censo de cargas CE3 para E-04
            sheetName: 'R. CARG. E4. CE3',
            labelsRange: 'I6:I19',
            dataRange: 'J6:J19',
            pieDataRange: 'K6:K19',
          }
        ...
        if (selectedStation) {
            ...
            try {
                ...
                if (response.data.status === 'ok') {
                    ...
                    if ( (selectedStation === 'E-01' || selectedStation === 'E-04') && stationConfig.censoCargas) {
                        const censoCargasConfig = stationConfig.censoCargas;
                        const sheetNameCensoCargas = censoCargasConfig.sheetName;
                        console.log(`[Debug E-04] Leyendo hoja para CE2: ${sheetNameCensoCargas}`); // DEBUG
                        const worksheetCensoCargas = workbook.Sheets[sheetNameCensoCargas];

                        if (worksheetCensoCargas) {
                            console.log(`[Debug E-04] Hoja "${sheetNameCensoCargas}" encontrada.`); // DEBUG
                            const rawLabels = XLSX.utils.sheet_to_json(worksheetCensoCargas, { range: censoCargasConfig.labelsRange, header: 1 });
                            const labels = rawLabels.map(row => row[0]);

                            const rawData = XLSX.utils.sheet_to_json(worksheetCensoCargas, { range: censoCargasConfig.dataRange, header: 1 });
                            const data = rawData.map(row => row[0] ?? 0);
                            console.log('[Debug E-04] Datos de barra extraídos:', { labels, data }); // DEBUG

                            setCensoCargasBarData({
                                labels: labels,
                                datasets: [
                                    {
                                        label: 'Cantidad',
                            ...
                            const rawPieData = XLSX.utils.sheet_to_json(worksheetCensoCargas, { range: censoCargasConfig.pieDataRange, header: 1 });
                            const pieData = rawPieData.map(row => row[0] ?? 0);
                            console.log('[Debug E-04] Datos de torta extraídos:', pieData); // DEBUG

                            const total = pieData.reduce((sum, val) => sum + val, 0);
                            const percentages = pieData.map(val => (val / total * 100).toFixed(0));

                            setCensoCargasPieData({
                                labels: labels,
                                datasets: [
                                    {
                                        data: percentages,
                            ...
                            console.error(`[Debug E-04] La hoja "${sheetNameCensoCargas}" no se encontró.`); // DEBUG
                            setCensoCargasBarData(null);
                            setCensoCargasPieData(null); // Limpiar también el estado del gráfico de torta
                        }
                    } else {
                        setCensoCargasBarData(null);
                        setCensoCargasPieData(null); // Limpiar también el estado del gráfico de torta
                    ...
                    // --- PROCESAR HOJA DE CENSO DE CARGAS CE3 (NUEVO) ---
                    if ((selectedStation === 'E-01' || selectedStation === 'E-04') && stationConfig.censoCargasCE3) {
                        const censoCargasCE3Config = stationConfig.censoCargasCE3;
                        const sheetNameCensoCargasCE3 = censoCargasCE3Config.sheetName;
                        const worksheetCensoCargasCE3 = workbook.Sheets[sheetNameCensoCargasCE3];

                        if (worksheetCensoCargasCE3) {
                            const rawLabelsCE3 = XLSX.utils.sheet_to_json(worksheetCensoCargasCE3, { range: censoCargasCE3Config.labelsRange, header: 1 });
                            const labelsCE3 = rawLabelsCE3.map(row => row[0]);

                            const rawDataCE3 = XLSX.utils.sheet_to_json(worksheetCensoCargasCE3, { range: censoCargasCE3Config.dataRange, header: 1 });
                            const dataCE3 = rawDataCE3.map(row => row[0] ?? 0);

                            setCensoCargasCE3BarData({
                                labels: labelsCE3,
                                datasets: [
                                    {
                                        label: 'Cantidad',
                            ...
                            // Preparar datos para el gráfico de torta CE3
                            const rawPieDataCE3 = XLSX.utils.sheet_to_json(worksheetCensoCargasCE3, { range: censoCargasCE3Config.pieDataRange, header: 1 });
                            const pieDataCE3 = rawPieDataCE3.map(row => row[0] ?? 0);

                            const totalCE3 = pieDataCE3.reduce((sum, val) => sum + val, 0);
                            const percentagesCE3 = pieDataCE3.map(val => (val / totalCE3 * 100).toFixed(0));

                            setCensoCargasCE3PieData({
                                labels: labelsCE3,
                                datasets: [
                                    {
                                        data: percentagesCE3,
                                        backgroundColor: labelsCE3.map((_, i) => `hsl(${((i + 60) * 360) / labelsCE3.length}, 70%, 50%)`),
                                        borderColor: '#fff',
                                        borderWidth: 1,
                                    },
                                ],
                            ...
                            console.error(`La hoja "${sheetNameCensoCargasCE3}" no se encontró.`);
                            setCensoCargasCE3BarData(null);
                            setCensoCargasCE3PieData(null);
                        }
                    } else { // Limpiar estados de CE3 si no es E-01
                        setCensoCargasCE3BarData(null);
                        setCensoCargasCE3PieData(null);
                    }
                } else {
                ...
                setOrigenDestinoData(null);
                setOrigenDestinoDataPesados(null);
                setCensoCargasBarData(null); // Limpiar también el nuevo estado en caso de error
                setCensoCargasPieData(null); // Limpiar también el estado del gráfico de torta en caso de error
                setCensoCargasCE3BarData(null);
                setCensoCargasCE3PieData(null);
            } finally {
            ...
            setOrigenDestinoData(null);
            setOrigenDestinoDataPesados(null);
            setCensoCargasBarData(null); // Limpiar también el nuevo estado si no hay estación seleccionada
            setCensoCargasPieData(null); // Limpiar también el estado del gráfico de torta si no hay estación seleccionada
            setCensoCargasCE3BarData(null);
            setCensoCargasCE3PieData(null);
            ...
            <div style={{
              ...
              <div style={{ border: '1px solid #eee', padding: '15px', borderRadius: '5px', minHeight: '100px', overflowX: 'auto' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '20px', marginTop: '20px' }}>
                  {(selectedStation === 'E-01' || selectedStation === 'E-04') && censoCargasBarData ? (
                    <div style={{ position: 'relative', border: '1px solid #ccc', padding: '10px', borderRadius: '5px' }}>
                      <Bar data={censoCargasBarData} options={{
                        responsive: true,
                        plugins: {
                          title: {
                            display: true,
                            text: 'PRODUCTOS DE CAMION E-2',
                            font: { size: 16 }
                          },
                          datalabels: {
                            display: true,
                  ...
                  ) : (selectedStation === 'E-01' || selectedStation === 'E-04') ? (
                    <div style={{ textAlign: 'center', padding: '20px', color: '#666' }}>
                      No se encontraron datos de censo de cargas para la Estación {selectedStation}.
                    </div>
                  ) : null}

                  {(selectedStation === 'E-01' || selectedStation === 'E-04') && censoCargasPieData ? (
                    <div style={{ position: 'relative', border: '1px solid #ccc', padding: '10px', borderRadius: '5px' }}>
                      <Pie data={censoCargasPieData} options={{
                        responsive: true,
                        plugins: {
                          title: {
                            display: true,
                          ...
                          percentageText: percentageTextPlugin // Aplicar el plugin directamente aquí
                        },
                      }} />
                    </div>
                  ) : (selectedStation === 'E-01' || selectedStation === 'E-04') ? (
                  ...
                  {(selectedStation === 'E-01' || selectedStation === 'E-04') && censoCargasCE3BarData ? (
                    <div style={{ position: 'relative', border: '1px solid #ccc', padding: '10px', borderRadius: '5px' }}>
                      <Bar data={censoCargasCE3BarData} options={{
                        responsive: true,
                        plugins: {
                          title: {
                            display: true,
                            text: 'PRODUCTOS DE CAMION E-3',
                            font: { size: 16 }
                          },
                          datalabels: {
                            display: true,
                    ...
                    <div style={{ textAlign: 'center', padding: '20px', color: '#666' }}>
                      No se encontraron datos de censo de cargas CE3 para la Estación {selectedStation}.
                    </div>
                  ) : null}

                  {(selectedStation === 'E-01' || selectedStation === 'E-04') && censoCargasCE3PieData ? (
                    <div style={{ position: 'relative', border: '1px solid #ccc', padding: '10px', borderRadius: '5px' }}>
                      <Pie data={censoCargasCE3PieData} options={{
                        responsive: true,
                        plugins: {
                          title: {
                            display: true,
