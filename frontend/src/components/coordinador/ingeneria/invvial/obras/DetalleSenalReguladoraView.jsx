...
const DetalleSenalReguladoraView = ({ senalReguladora: senal, images, route, onCloseDetail }) => {
    ...
    const handleExportPDF = async () => {
        ...
        try {
          ...
          `,
            ...
            const titleId = senal.codigo || 'Sin Código';
            // Center title
            pdf.text(`Señal Reguladora: ${titleId}`, pdfWidth / 2, cursorY, { align: 'center' });
            cursorY += 10;
            ...
            pdf.save(`Senal_Reguladora_${titleId}.pdf`);
        ...
        <div style={{
            ...
            <div ref={modalRef} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #eee', paddingBottom: '2px' }}>
                    <h2 style={{ margin: 0, color: '#333', fontSize: '1.1rem' }}>Señal Reguladora: {senal.codigo || 'Sin Código'}</h2>
                ...
                <div style={{ display: 'flex', gap: '10px', flex: 1 }}>
                    ...
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                        <div ref={dataRef} style={{
                            ...
                            <p style={{ margin: '3px 0' }}><strong>Código:</strong> {senal.codigo || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Progresiva:</strong> {senal.progresiva || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Tipo:</strong> {senal.tipo || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Clasificación:</strong> {senal.clasificacion || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Lado:</strong> {senal.lado || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Soporte:</strong> {senal.soporte || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Material:</strong> {senal.material || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Entregable:</strong> {senal.entregable || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Coordenadas:</strong> {senal.latitud ? `${senal.latitud.toFixed(6)}, ${senal.longitud.toFixed(6)}` : 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Observaciones:</strong> {senal.observaciones || '---'}</p>
                        </div>
                    </div>
                    ...
                    <div style={{ flex: 2, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        ...
                        <div ref={mapRef} style={{
                            ...
                            {/* Reuse MiniMap BUT pass senal as 'alcantarilla' prop because component expects it */}
                            <MiniMap alcantarilla={senal} route={route} />
                        </div>
...
export default DetalleSenalReguladoraView;
