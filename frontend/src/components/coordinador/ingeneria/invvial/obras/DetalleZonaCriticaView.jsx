...
const DetalleZonaCriticaView = ({ zonaCritica, images, route, onCloseDetail }) => {
    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            const titleId = zonaCritica.codigo || zonaCritica.id_zona_critica;
            // Center title
            pdf.text(`Zona Crítica: ${titleId}`, pdfWidth / 2, cursorY, { align: 'center' });
            cursorY += 10;
            ...
            pdf.save(`ZonaCritica_${titleId}.pdf`);
    ...
    if (!zonaCritica) {
        return null; // No renderizar si no hay zona seleccionada
    }
        ...
        <div style={{
            ...
            <div ref={modalRef} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #eee', paddingBottom: '2px' }}>
                    <h2 style={{ margin: 0, color: '#333', fontSize: '1.1rem' }}>Zona Crítica: {zonaCritica.codigo || zonaCritica.id_zona_critica}</h2>
                ...
                <div style={{ display: 'flex', gap: '10px', flex: 1 }}>
                    ...
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                        <div ref={dataRef} style={{
                            ...
                            <p style={{ margin: '3px 0' }}><strong>Progresiva:</strong> {(() => {
                                const value = zonaCritica.progresiva;
                                if (!value) return 'N/A';
                            ...
                            <p style={{ margin: '3px 0' }}><strong>Código:</strong> {zonaCritica.codigo || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Tipo:</strong> {zonaCritica.tipo || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Clase de Daño:</strong> {zonaCritica.clase_dano || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Condición:</strong> {zonaCritica.condicion || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Lado:</strong> {zonaCritica.lado || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Longitud:</strong> {zonaCritica.longitud_zona || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Altitud:</strong> {zonaCritica.altitud || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Coordenadas:</strong> {zonaCritica.latitud ? `${zonaCritica.latitud.toFixed(6)}, ${zonaCritica.longitud.toFixed(6)}` : 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Observaciones:</strong> {zonaCritica.observaciones || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Panel Fotográfico:</strong> {zonaCritica.panel_fotografico_codigo || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Entregable:</strong> {zonaCritica.entregable || 'N/A'}</p>
                        </div>
                    </div>
                    ...
                    <div style={{ flex: 2, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        ...
                        <div ref={mapRef} style={{
                            ...
                            {/* Reusing MiniMap, passing zonaCritica as alcantarilla prop (it expects 'alcantarilla' but uses lat/lon/custom props usually) */}
                            {/* Assuming MiniMap is generic enough or I might need to verify it */}
                            {/* MiniMap accepts 'alcantarilla' prop. Let's send zonaCritica there. */}
                            <MiniMap alcantarilla={zonaCritica} route={route} />
                        </div>

                        {/* Visualizador Multimedia */}
                        <ImageCarousel images={images} alcantarillaId={zonaCritica.id_zona_critica} />
                    </div>
...
export default DetalleZonaCriticaView;
