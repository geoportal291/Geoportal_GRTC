...
import DetalleMuroView from './DetalleMuroView'; // Importar la nueva vista de detalle
...
const ListaMurosModal = ({ show, onClose, murosData, route, graphicsImages, initialSelectedMuro }) => {
  ...
  const [selectedMuroDetail, setSelectedMuroDetail] = useState(null);
  ...
  useEffect(() => {
    ...
    if (show) {
      ...
      if (initialSelectedMuro) {
        handleRowClick(initialSelectedMuro);
      } else {
        setShowDetailView(false);
        setSelectedMuroDetail(null);
        setDetailedImages([]);
      }
    }
    return () => {
  ...
  }, [show, onClose, initialSelectedMuro]);

  const handleRowClick = (muro) => {
    setSelectedMuroDetail(muro);

    if (muro && muro.panel_fotografico_codigo && graphicsImages) {
      const code = String(muro.panel_fotografico_codigo);
        ...
        <div ref={modalRef} className="hide-scrollbar" style={{
          ...
          {murosData && murosData.length > 0 ? (
              ...
              <table className="invvial-table">
                ...
                <tbody>
                  {murosData.sort((a, b) => {
                    ...
                    <tr key={element.id_muro} className="alcantarilla-row" onClick={() => handleRowClick(element)}>
            ...
            <div style={{ textAlign: 'center', color: '#777', padding: '20px' }}>
              <p>No hay muros para mostrar.</p>
            </div>
          )}
        </div>
      ) : (
        <DetalleMuroView
          alcantarilla={selectedMuroDetail}
