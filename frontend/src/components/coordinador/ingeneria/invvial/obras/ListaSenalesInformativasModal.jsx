...
import DetalleSenalInformativaView from './DetalleSenalInformativaView';
import './ListaSenalesInformativasModal.css'; // New CSS file

const ListaSenalesInformativasModal = ({ show, onClose, senalesData, graphicsImages, initialSelection }) => {
    ...
    const parseProgresivaValue = (val) => {
        if (!val) return 0;
        // Remove 'KM', '+' and spaces to get raw number
    ...
    // parseProgresivaValue helper is already defined above
    ...
    const formatProgresiva = (prog) => {
        if (!prog) return '';
        // If it's already in KM format, return as is
    ...
    return (
        <div className="lista-senales-modal-overlay">
            <div className="lista-senales-modal-content">
                ...
                {!selectedSenal ? (
                    <>
                        <h2 className="lista-senales-modal-title" style={{ textAlign: 'center' }}>Lista de Señales Informativas</h2>
...
export default ListaSenalesInformativasModal;
