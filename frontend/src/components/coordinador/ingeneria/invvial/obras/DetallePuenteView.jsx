...
const DetallePuenteView = ({ alcantarilla: puente, images, route, onCloseDetail }) => {
    ...
    const handleExportPDF = async () => {
        ...
        header.innerText = `Puente: ${puente.nombre || puente.id_puente}`;
    ...
    if (!puente) {
        return null; // No renderizar si no hay puente seleccionado
    }
        ...
        <div style={{
            ...
            <div ref={modalRef} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #eee', paddingBottom: '2px' }}>
                    <h2 style={{ margin: 0, color: '#333', fontSize: '1.1rem' }}>Puente: {puente.nombre || puente.id_puente}</h2>
                ...
                <div style={{ display: 'flex', gap: '10px', flex: 1 }}>
                    ...
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                        <div ref={dataRef} style={{
                            ...
                            <p style={{ margin: '3px 0' }}><strong>Progresiva:</strong> {puente.progresiva || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Nombre:</strong> {puente.nombre || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Clase:</strong> {puente.clase || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Tipo:</strong> {puente.tipo || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Estado:</strong> {puente.estado || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Longitud:</strong> {puente.longitud_puente || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Ancho:</strong> {puente.ancho || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Coordenadas:</strong> {puente.latitud ? `${puente.latitud.toFixed(6)}, ${puente.longitud.toFixed(6)}` : 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Observaciones:</strong> {puente.observaciones || 'N/A'}</p>
                            <p style={{ margin: '3px 0' }}><strong>Código de Panel Fotográfico:</strong> {puente.panel_fotografico_codigo || 'N/A'}</p>
                        </div>
                    </div>
                    ...
                    <div style={{ flex: 2, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        ...
                        <div ref={mapRef} style={{
                            ...
                            <MiniMap alcantarilla={puente} route={route} />
                        </div>

                        {/* Visualizador Multimedia */}
                        <ImageCarousel images={images} alcantarillaId={puente.id_puente} />
                    </div>
                ...
                <div className="no-print" style={{ borderTop: '2px solid #eee', paddingTop: '10px', textAlign: 'center', display: 'flex', justifyContent: 'center', gap: '10px' }}>
                    ...
                    <button onClick={onCloseDetail} style={{
                        ...
                        fontWeight: 'bold',
                        transition: 'background-color 0.2s ease'
                    }} onMouseOver={(e) => e.currentTarget.style.color = '#333'} onMouseOut={(e) => e.currentTarget.style.color = '#007bff'}>Regresar a lista</button>
                </div>
            </div>
        </div>
    );
...
export default DetallePuenteView;