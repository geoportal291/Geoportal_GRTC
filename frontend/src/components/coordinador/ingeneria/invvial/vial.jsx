...
const Vialds = ({ isNavbarExpanded }) => {
  ...
  const fetchVialData = useCallback(async () => {
    ...
    try {
      const [alcantarillasRes, graphicsRes] = await Promise.all([
        axiosInstance.get(`/api/proyectos/${projectId}/alcantarillas`, {
          headers: { Authorization: `Bearer ${user.token}` }
        }),
        axiosInstance.get(`/api/alcantarillas/graphics/${projectId}`, {
          headers: { Authorization: `Bearer ${user.token}` }
        })
      ]);
  ...
  const handleSaveManualAlcantarilla = useCallback(async (newAlcantarillaData) => {
    console.log('Datos de alcantarilla a guardar:', newAlcantarillaData);
    if (!user || !user.token) {
      console.warn('Usuario no autenticado o token no disponible.');
      return;
    }
    ...
    try {
      let response;
      if (newAlcantarillaData.id_alcantarilla) {
        response = await axiosInstance.put(`/api/alcantarillas-e1/${newAlcantarillaData.id_alcantarilla}`, newAlcantarillaData, {
          headers: { Authorization: `Bearer ${user.token}` }
        });
        console.log('Alcantarilla actualizada exitosamente:', response.data);
      } else {
        response = await axiosInstance.post('/api/alcantarillas-e1', newAlcantarillaData, {
          headers: { Authorization: `Bearer ${user.token}` }
        });
        console.log('Alcantarilla guardada exitosamente:', response.data);
      }
      fetchVialData();
    } catch (error) {
      console.error('Error al guardar/actualizar la alcantarilla:', error);
    } finally {
      setIsLoadingData(false);
        ...
        return (
          <>
            ...
            {activeObrasSubTab === 'BADENES' && (
              <div><h3>Contenido de BADENES</h3></div>
            )}
