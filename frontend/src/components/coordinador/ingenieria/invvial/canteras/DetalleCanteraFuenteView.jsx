...
const DetalleCanteraFuenteView = ({ element, images, route, onCloseDetail }) => {
    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            const incrementStep = (message) => {
                completedSteps++;
                // Scale progress to mostly fill bar but leave room for final save
            ...
            const titleType = element.type === 'cantera' ? 'Cantera' : 'Fuente de Agua';
            const titleId = element.item_number || element.id;
            pdf.text(`${titleType}: ${titleId}`, pdfWidth / 2, cursorY, { align: 'center' });
            cursorY += 10;
            ...
            if (images && images.length > 0) {
                ...
                let imageStepRange = 65; // Cover from 30% to 95%
            ...
            // Allow text to be seen briefly
            await new Promise(r => setTimeout(r, 500));

            pdf.save(`${element.type === 'cantera' ? 'Cantera' : 'Fuente'}_${titleId}.pdf`);
        ...
        <div style={{
            ...
            <div ref={modalRef} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #eee', paddingBottom: '2px' }}>
                    <h2 style={{ margin: 0, color: '#333', fontSize: '1.1rem' }}>
                        {element.type === 'cantera' ? 'Cantera' : 'Fuente de Agua'} {element.item_number || ''}
                    </h2>
                ...
                <div style={{ display: 'flex', gap: '10px', flex: 1 }}>
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                        <div ref={dataRef} style={{
                            ...
                            border: '1px solid #eee', flex: 1, overflowY: 'auto', fontSize: '0.9rem',
                            display: 'flex', flexDirection: 'column', justifyContent: 'center'
                        }}>
                            <h4 style={{ margin: '0 0 8px 0', color: '#333', fontSize: '1rem' }}>Datos Técnicos</h4>
                            <p style={{ margin: '3px 0' }}><strong>Entregable:</strong> {element.entregable}</p>
                            <p style={{ margin: '3px 0' }}><strong>Progresiva:</strong> {element.progresiva}</p>
                            <p style={{ margin: '3px 0' }}><strong>Panel Fotográfico:</strong> {element.panel_fotografico}</p>
                            <p style={{ margin: '3px 0' }}><strong>Coordenadas:</strong> {element.latitud}, {element.longitud}</p>
                            <p style={{ margin: '3px 0' }}><strong>Altitud:</strong> {element.altitud}</p>
                            <p style={{ margin: '3px 0' }}><strong>Lado:</strong> {element.lado}</p>
                            <p style={{ margin: '3px 0' }}><strong>Propietario:</strong> {element.propietario}</p>
                            {element.type === 'fuente' && (
                                <p style={{ margin: '3px 0' }}><strong>Ubicación:</strong> {element.ubicacion}</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div style={{ flex: 2, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        <div ref={mapRef} style={{ width: '100%', height: '150px', borderRadius: '8px', overflow: 'hidden' }}>
                            <MiniMap alcantarilla={element} route={route} />
                        </div>
                        <ImageCarousel images={images} alcantarillaId={element.id} />
                    </div>
                </div>
...
export default DetalleCanteraFuenteView;
